      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {users
          ...
          .map((user) => (
            <div
              key={user?._id}
              className="flex flex-col  rounded-3xl shadow-lg   border-3 border-gray-500 "
            >
              <div className="px-6 py-8 sm:p-10 sm:pb-6">
                <div className="grid items-center justify-center w-full text-left">
                  <div>
                    <h2 className="text-lg font-medium tracking-tighter text-amber-50 lg:text-2xl">
                      {user?.name}
                    </h2>
                    <p className="mt-2 text-sm text-amber-50">{user?.email}</p>
                  </div>
                </div>
              </div>
